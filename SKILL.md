...
| `references/control-lineage.md` | **What a requirement means or why it exists** — SP 800-53 Rev 4/Rev 5 parentage, tailoring criteria, ODPs, interpreting vague requirement text. Pair with the bundled mapping + parent-control text in `references/sources/` (below) |
...
Two of these carry the questions that most often get answered badly. Reach for `control-lineage.md` whenever the question is *what does this requirement actually mean* rather than *what does it say* — the SP 800-53 parent control and its discussion text are where NIST recorded the intent that 800-171's compressed phrasing left out. That parent text is bundled: `references/sources/sp-800-171r2-to-800-53-mapping.md` maps each requirement to its 800-53 control(s), and `references/sources/sp-800-53-rev5-cui-parents.md` holds those controls' verbatim text and Discussion. An interpretive answer is not finished until you have quoted the parent's Discussion. Reach for `cui-program.md` whenever the question is about the information rather than the system — CMMC verifies protection, but the CUI Program decides what must be protected, who designates it, and what handling rules ride along. A consultant who knows only CMMC will give confidently wrong answers about marking, Specified categories, and unmarked data.

### Bundled primary sources — go here for exact text

`references/sources/` holds the full text of the seven documents this skill is built on, extracted directly from the government PDFs and official machine-readable catalogs (not paraphrased):
...
| `references/sources/sp-800-171r2-to-800-53-mapping.md` | SP 800-171 Rev 2 Appendix D (Tables D-1–D-14) — the parent SP 800-53 control(s) behind each of the 110 CUI requirements. Read this to know **which** control to interpret from |
| `references/sources/sp-800-53-rev5-cui-parents.md` | Verbatim *Control* text and *Discussion* for every SP 800-53 Rev 5 control that is a parent of a Level 2 requirement (~126 controls/enhancements), from NIST's OSCAL catalog. Read this for **what the parent control says** |
...
1. **Find the parent control.** `references/sources/sp-800-171r2-to-800-53-mapping.md` gives the SP 800-53 control(s) behind every one of the 110 requirements (from SP 800-171 Rev 2 Appendix D).
2. **Read its text.** `references/sources/sp-800-53-rev5-cui-parents.md` carries the verbatim Control statement and Discussion for every one of those parent controls. If you need a control outside that set (Level 3 / 800-172 lineage, a related control named in a Discussion), fetch it from the NIST CPRT catalog (`csrc.nist.gov/projects/cprt`) or SP 800-53 Rev 5 on `csrc.nist.gov`, and say you did.
3. **Quote and cite it** by control ID and revision (e.g. "AC-2 Discussion, SP 800-53 Rev 5"), and reason from its intent — but assess against the 800-171 requirement and its 800-171A objectives, never against obligations 800-53 carries that 800-171 did not adopt.

An interpretive answer that never names or quotes the parent 800-53 control is incomplete. SP 800-171 Rev 2 was tailored from Rev 4 and the bundled parent text is Rev 5; identifiers and intent are stable for the base controls, but note the revision where wording or an enhancement number diverges. `control-lineage.md` covers the method and the worked examples.
