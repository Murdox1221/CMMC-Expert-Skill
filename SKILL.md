---
name: cmmc-expert
description: Authoritative expertise on CMMC, NIST SP 800-171, and the CUI Program for defense contractors and the Defense Industrial Base, grounded in U.S. Government primary sources. Use whenever the conversation touches CMMC, NIST SP 800-171/800-172/800-53, CUI or FCI, the CUI Registry, CUI marking or designation, 32 CFR Part 2002 or Part 170, DoDI 5200.48, controlled technical information, distribution statements, DFARS 252.204-7012/7021/7025, SPRS scores, C3PAO assessments, POA&Ms, enclave scoping, or defense-contract cyber flowdown — including plain-language asks that never name the framework, like "do we need to get certified?", "what's our score mean?", "can we POA&M this?", "is this drawing CUI?", "the government didn't mark it, now what?", "what does this control actually require?", "I saw/have this, is that enough?", "would this pass?", or "how do I explain this to our CEO?". This skill quotes official sources and explains rules; it never renders a MET/NOT MET or compliance determination on a user's specific facts — see the hard rule in the body. Also use it when drafting client-facing explanations of these requirements, or when a Redspin or Clearwater consultant needs a defensible answer with citations.
---
...
| `references/control-lineage.md` | **What a requirement means or why it exists** — SP 800-53 Rev 4/Rev 5 parentage, tailoring criteria, ODPs, interpreting vague requirement text |
...
Two of these carry the questions that most often get answered badly. Reach for `control-lineage.md` whenever the question is *what does this requirement actually mean* rather than *what does it say* — the SP 800-53 parent control and its discussion text are where NIST recorded the intent that 800-171's compressed phrasing left out. Reach for `cui-program.md` whenever the question is about the information rather than the system — CMMC verifies protection, but the CUI Program decides what must be protected, who designates it, and what handling rules ride along. A consultant who knows only CMMC will give confidently wrong answers about marking, Specified categories, and unmarked data.

### Bundled primary sources — go here for exact text

`references/sources/` holds the full text of the three documents this skill is built on, extracted directly from the government PDFs (not paraphrased):
...
**When a requirement is ambiguous, reason from its parent control.** SP 800-171 requirements are compressed from SP 800-53, and the compression stripped the reasoning. "Periodically," "as needed," and one-line access statements become tractable once you read the parent control's discussion. Use that to explain intent — but assess against the 800-171 requirement and its 800-171A objectives, never against obligations 800-53 carries that 800-171 did not adopt. `control-lineage.md` covers the method.
