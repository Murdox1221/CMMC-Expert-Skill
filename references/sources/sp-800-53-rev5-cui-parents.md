# SP 800-53 Rev 5 — Parent Controls for CUI Security Requirements (secondary lens)

**What this is.** Verbatim *Control* text and *Discussion* for every SP 800-53 **Rev 5** control/enhancement that is a parent of a CUI security requirement under **either** SP 800-171 Rev 2 (Appendix D) **or** SP 800-171 Rev 3 (NIST's Rev 3 → Rev 5 mapping). ~171 controls and enhancements.

**This is the secondary lens.** The default path for a CMMC Level 2 question is SP 800-171 Rev 2 → SP 800-53 **Rev 4** — see `references/sources/sp-800-53-rev4-cui-parents.md`. Reach for this Rev 5 file when:

- Rev 4's Supplemental Guidance is thin or ambiguous and you want Rev 5's fuller Discussion on the same control;
- the question touches SP 800-171 Rev 3, SP 800-53 Rev 5, or ODPs;
- you want to see how NIST restated the parent control's intent in the current catalog.

Pair it with `references/sources/sp-800-171r3-to-800-53r5-mapping.md`. SP 800-171 Rev 3 requirement `03.xx.xx` corresponds to Rev 2's `3.x.x` by sort-id; 33 of Rev 2's 110 requirements were consolidated in Rev 3, so some Rev 2 IDs have no direct Rev 3 row — use the Rev 4 default for those.

**Source.** NIST's official OSCAL catalog for **SP 800-53 Rev 5.2.0** (`usnistgov/oscal-content`, last modified 2026-05-11). Not paraphrased. `[Assignment: …]` / `[Selection: …]` are NIST's organization-defined parameter placeholders. Seven enhancements SP 800-171 Rev 2 cites are marked **WITHDRAWN** here, with the Rev 5 control they moved to.

**Assess against SP 800-171, not against these.** Interpretation only; never import an obligation SP 800-171 did not adopt.
